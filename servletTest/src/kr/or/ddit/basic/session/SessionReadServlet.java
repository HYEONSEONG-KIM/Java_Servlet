...
public class SessionReadServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		out.println("<body>"); 
		out.println("<h2>저장된Session 데이터 확인하기</h2><hr>");
		
		out.println("<h3>Session 데이터 1개 확인하기</h3>");
		// 2.Session객체의 getAttribute()메서드 이용
		...
		if(sessionValue == null) {
			...
			out.println("<h4>testSession의 세션값 : " + sessionValue +" 입니다</h4>");
		}
		out.println("<hr>");
		...
		while(sessionNames.hasMoreElements()) {
			cnt++;
			String sessionKey = sessionNames.nextElement();	// 세션 Key값 1개 구하기
