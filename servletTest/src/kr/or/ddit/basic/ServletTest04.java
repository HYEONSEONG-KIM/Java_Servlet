...
public class ServletTest04 extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		/*
		 * Servlet클래스나 JSP 페이지의 환경에 관련된 정보는 javax.servlet.ServletContext인터페이스 타입의
		 * 객체를 이용해서 얻을 수 있음
		 * 
		 * ServletContext객체의 메서드
		 * - getServerInfo()메서드 => Servlet이 속하는 웹 서버의 종류를 반환
		...
		/*
		 * 웹 애플리케이션의 초기화 파라미터(web.xml에 설정해서 사용)
		 * 	=> 특정 Servlet에 속하는 초기화 파라미터가 아니라 웹 애플리케이션 전체에 속하는 파라미터를 말함
		 * 		
		 * web.xml 작성 방법
		 * <web-app>
		 * 	
		 ...
		 * 
		 * 초기화 파라미터값 가져오기
		 * 	- ServletContext객체를 구하고, 이 객체의 getInitParameter()메서드를 호출해서 사용
		 * 	- JSP페이지에서는 application 내장 객체 변수의 getInitParameter()메서드를 호출해서 가져옴
		 * 	- getInitParameter()메서드로 가져온 값은 무조건 문자열로 가져옴
		 * 
		 * 
		 */
		out.println("<hr>");
		String uploadPath = context.getInitParameter("Upload_Path");
		out.printf("wed.xml에 설정된 파라미터 : %s<br>",uploadPath);
		
		/*
		 * Q) 초기화 파라미터로 다음 값들을 등록해 놓고 이 값을 읽어와 합계를 구하는 프로그램을 작성
